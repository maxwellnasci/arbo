...
| **Melhorias de Fluxo (Task 63)** | ✅ Campo de nome incluído na tela de cadastro (`SetPassword.tsx`), reprodução de vídeo expansível (inline) adicionada na biblioteca de treinos (`TreinoCard`) e painel lateral de turma (`SidePanel`) (2026-06-29) |
| **Editar Perfil Admin (Task 64)** | ✅ Edição de nome do aluno no admin, com email extraído de `auth.users` via RPC e exibido corretamente no perfil (2026-06-29) |
