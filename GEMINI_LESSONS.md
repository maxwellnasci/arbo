...
### 16. Keep-alive e healthchecks em Supabase: pingar tabela de negócio sob RLS gera 42501, não sinal de saúde (2026-09-21)
**O que aconteceu:** o workflow `keep-alive.yml` pingava `GET /rest/v1/training_types` para forçar atividade no Postgres do free tier. Como a role `anon` não tem `GRANT` nessa tabela, a resposta era sempre `401`/`42501 permission denied` — um "sucesso esperado" que misturava falso-positivo de permissão com queda real de infraestrutura: toda leitura do log exigia lembrar que 401 = saudável, e uma mudança futura de GRANT/policy poderia inverter o sinal sem ninguém perceber.
**Como evitar:** healthcheck de infra deve bater num endpoint dedicado que retorna 200 quando o banco está vivo, nunca numa tabela de negócio. Arquitetura recomendada: RPC pública dedicada `public.keepalive()` retornando `{"status":"ok"}` (`RETURNS jsonb`, `LANGUAGE sql`, `SECURITY INVOKER`, `SET search_path = ''`, corpo qualificado em `pg_catalog`, `GRANT EXECUTE ... TO anon, authenticated, service_role`), chamada via `POST /rest/v1/rpc/keepalive`. `SECURITY INVOKER` (nunca `DEFINER` num endpoint anônimo) + `search_path` vazio satisfazem o advisor de segurança do Supabase sem escalação de privilégio.

### 17. Ordem de deploy em workflows estritos: migration antes do workflow que a exige (2026-09-21)
**O que aconteceu:** ao endurecer o `keep-alive.yml` para exigir HTTP 200 do `POST /rpc/keepalive`, o workflow passou a depender de uma função que só existe no banco depois da migration `20260921000000_add_keepalive_rpc.sql` aplicada. Se o workflow rodar (schedule ou dispatch manual) antes do `db push`, falha vermelho com 404 — um falso-negativo de "queda" causado só por deploy fora de ordem.
**Como evitar:** workflows que exigem HTTP 200 de endpoints novos devem ser commitados e ativados em sincronia ou **após** a aplicação da migration no banco (`npx supabase db push` primeiro, `workflow_dispatch` de validação depois). Regra geral: o lado que consome um contrato novo nunca entra em produção estrita antes do lado que o provê.
