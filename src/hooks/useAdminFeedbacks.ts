  ...
  useEffect(() => {
    ...
    async function fetchFeedbacks() {
      ...
      const [{ data: checkins, error: checkinError }, { data: records }] = await Promise.all([
        supabase
          .from('checkins')
          .select('id, training_id, actual_distance_m, actual_duration_seconds, actual_pace_seconds_per_km, perceived_effort, approved, approved_by, completed_at, created_at, notes, plan_id, strava_activity_id, professor_feedback, professor_feedback_at, professor_feedback_seen_at, student_id, profiles!checkins_student_id_fkey(id, full_name, avatar_url, birth_date, group_id, has_set_password, level, role, strava_athlete_id, created_at, updated_at)')
