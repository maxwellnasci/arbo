  ...
  useEffect(() => {
    ...
    async function load() {
      const [trainingsRes, tagsRes, typesRes] = await Promise.all([
        supabase.from('trainings').select('id, title, duration_minutes, distance_m, type, description, sets, target_pace_seconds_per_km, tag_id, created_at, created_by, updated_at').order('title').limit(200),
