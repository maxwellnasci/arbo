...
- **Upload de Vídeo (Cloudflare R2):** bucket `arbo-videos` (10GB grátis) + domínio `videos.mxos.com.br`. Edge Function `r2-upload` gera **presigned URL** (SigV4/`aws4fetch`) em vez de proxiar os bytes — Edge Functions não aguentam upload de até 500MB. `TreinoFormPanel.tsx` ganhou toggle YouTube/Upload com barra de progresso (`XMLHttpRequest`); `VideoPlayer.tsx` detecta R2 vs YouTube pela URL. `vercel.json` — CSP com `media-src`/`connect-src` liberados. ✅ **100% CONCLUÍDO E VALIDADO EM PRODUÇÃO (2026-09-22):** regras de CORS aplicadas no bucket `arbo-videos` e validadas ponta a ponta com upload e reprodução via `videos.mxos.com.br`.
