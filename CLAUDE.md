...
- **`SetPassword.tsx`** trata erro de link expirado (`#error=access_denied&error_code=otp_expired` no hash da URL) **antes** de aguardar a sessão do Supabase — sem isso, um aluno clicando num link de convite/recuperação já expirado ficava preso na tela "Validando convite..." para sempre (o código esperava por uma sessão que nunca seria emitida). A checagem do hash fica isolada em `async function checkHash()` dentro do `useEffect` (mesmo padrão de "useEffect com fetch assíncrono" acima) — chamar `setState` síncrono no corpo do effect viola o lint `react-hooks/set-state-in-effect` no CI da Vercel (zero-tolerance), mesmo funcionando normalmente em dev local. Ver Caso 14 em `docs/PORTFOLIO_DEBUG_CASES.md`.
...
**Check-in integrado ao Strava (implementado 2026-07-12):** `checkins.strava_activity_id` (nullable) vincula um checkin a uma atividade já sincronizada. `useStravaActivitiesLocal.ts` (`src/hooks/`) é um hook **novo e deliberadamente separado** de `useStravaConnection` — lê `strava_activities` direto via RLS (`user_id` + `type='Run'`, últimas 10) em vez de reusar o hook existente, porque `useStravaConnection` dispara `strava-sync`/`strava-analyze` no mount (bateria a API do Strava e o DeepSeek só para abrir o bottom-sheet de check-in) e resolve o usuário pelo JWT da sessão em vez de aceitar `userId` explícito (quebraria no modo `/preview-aluno`, que passa um `userId` fixo diferente do `auth.uid()`). `CheckinSheet.tsx` ganhou botão "Importar do Strava": ao selecionar uma atividade da lista, `distance`/`minutes` são preenchidos automaticamente e os campos numéricos ficam `readOnly` — o aluno precisa clicar no "×" (desvincular) antes de conseguir editar manualmente, para não haver estado ambíguo entre "valor importado" e "valor editado por cima". `AlunoDashboard.tsx` calcula `usedStravaActivityIds` (atividades já vinculadas a outro checkin da semana) só para exibir o badge "já usada" na lista de importação — **não bloqueia** reimportar a mesma atividade.

**Fix — GRANT ausente quebrando a sincronização silenciosamente por 8 dias (2026-07-12):** desde a Task 68, `service_role` tinha `INSERT`/`UPDATE` em `strava_activities` mas não `SELECT`. Como `.upsert()` do PostgREST faz `RETURNING *` implícito, a falta de `SELECT` derrubava a escrita inteira sem erro visível — `strava-sync` respondia 200 ao cliente com um resumo montado em memória (não com o que foi de fato persistido), mascarando que a tabela ficava sempre vazia. Só foi descoberto quando a feature de check-in acima passou a *ler* a tabela em vez de confiar na resposta ao vivo da function. Corrigido com `GRANT INSERT, UPDATE ON strava_activities TO service_role` (migration `20260712233847`). Ver Caso 13 em `docs/PORTFOLIO_DEBUG_CASES.md`.
...
**CORS do bucket R2 — RESOLVIDO e testado em produção (2026-08-25):** o bucket já tinha CORS configurado (origens `arbo.mxos.com.br`, `*.vercel.app`, `localhost:5173`; métodos GET/PUT/HEAD; headers `*`; `MaxAgeSeconds 3600`) — upload real de vídeo testado e funcionando ponta a ponta.

**Exclusão do objeto no R2 ao remover vídeo — RESOLVIDO (2026-08-25):** Edge Function `r2-delete` (espelha `r2-upload`: JWT + `role=admin`, CORS allowlist, SigV4/`aws4fetch` `region:auto`, força prefixo `videos/` na key extraída da URL pública — defesa em profundidade contra path fora do bucket autorizado; trata 404 do R2 como sucesso, idempotente). Hook `src/hooks/useR2Delete.ts` chama a function direto (nunca `supabase.from`), `Sentry.captureException` em falha.

**Exclusão é destrutiva e irreversível — por isso o fluxo é adiado até o save confirmar, não fica no clique da lixeira:** clicar em remover no `TreinoFormPanel.tsx` só abre um `<ConfirmModal>` (via `createPortal` pro `document.body` — necessário porque o painel do formulário usa `motion.div` com `transform` na animação de slide-in, e `position: fixed` perde a viewport como containing block dentro de ancestral com `transform`, mesma classe do Caso 8/Sessão 2026-07-11 tarde) e, ao confirmar, só marca a URL antiga em `pendingDeleteUrl` (state local) — nenhuma chamada de rede acontece nesse momento. A Edge Function só é chamada de fato dentro de `AdminTreinos.tsx#handleSubmit`, **depois** que `updateTraining`/`createTraining` confirma sucesso: se o save falhar, o vídeo antigo continua intacto no bucket; se o delete falhar (save já OK), o pior caso é o arquivo ficar órfão (o problema original, bem mais barato que link quebrado). Motivo da ordem: a primeira versão da feature chamava `r2-delete` direto no clique da lixeira, antes do submit — se o admin clicasse "Cancelar" depois de apagar, o objeto já tinha sumido do R2 pra sempre (sem undo) mas o `video_url` no banco continuava intacto apontando pra ele, quebrando o vídeo em produção sem forma de reverter. `pendingDeleteUrl` é resetado toda vez que o painel reabre/troca de treino editado, então nada vaza entre edições.
...
Análise completa somente-leitura do projeto após ~2 semanas sem atividade, seguida de correções operacionais: **(1) Pausa do Supabase detectada e restaurada** — free tier pausa após 7 dias sem tráfego; sintomas no diagnóstico: DNS de `*.supabase.co` não resolvia (NXDOMAIN), REST/Auth inalcançáveis, porta 5432 em timeout — o `npx supabase login` funcionava (Management API) mas `migration list` morria na conexão direta ao banco. Restauração só via Dashboard (`Restore project`), CLI não tem comando. Após restore, confirmado zero drift: schema de produção **byte a byte idêntico** a `database.types.ts`, 9 migrations locais = 9 aplicadas. **(2) npm audit 10 → 0 vulnerabilidades** (`e754d15`) — `npm audit fix` resolveu 8; os 2 restantes exigiram major: **`react-router-dom` 6.30 → 7.18** (open redirect CVE; migração já estava agendada no histórico) e `sharp-cli` 5 → 6 (dev-only). Verificado pós-upgrade: tsc zero erros, lint limpo, 22 testes, build+PWA OK — API usada pelo app (`createBrowserRouter`/`useNavigate`/`Outlet`) é drop-in no v7. **Smoke test manual de navegação ainda recomendado** (rotas não têm cobertura de teste automatizado). **(3) Keep-alive anti-pausa** (`88c8f4c`) — `.github/workflows/keep-alive.yml`: ping diário 09:00 BRT em `/auth/v1/health` + `/rest/v1/` com secret `SUPABASE_ANON_KEY` (configurado via `gh secret set`; chave nova formato `sb_publishable_`). REST root responde 401 mesmo com key válida (comportamento do projeto com chaves novas), Auth responde 200 — para keep-alive tanto faz, o que conta é a requisição chegar. Workflow falha vermelho se conexão morrer (= projeto pausado). Ping previne pausa, não restaura. **(4) Token PAT removido da URL do git remote** — estava embutido em texto plano no `.git/config`; limpo com `git remote set-url` + `gh auth setup-git` (helper por host via keyring; `git ls-remote` validou auth). **(5) CORS do R2 confirmado configurado e upload testado em produção** — bucket já tinha a policy correta desde algum momento anterior não documentado.

### Sessão 2026-08-25 (tarde) — Exclusão do objeto R2 ao remover vídeo, com revisão sênior e correção de bug de dessincronia

Fecha a última pendência da seção anterior. Primeira versão (`824002e`): Edge Function `r2-delete` (espelha `r2-upload` — JWT + `role=admin`, CORS allowlist, SigV4/`aws4fetch`, prefixo `videos/` forçado na key) + hook `useR2Delete.ts` + wiring em `AdminTreinos.tsx`/`TreinoFormPanel.tsx`. Revisão sênior (comparando com o próprio código, não só o resumo da implementação) encontrou 2 problemas antes do deploy virar definitivo: **(1) exclusão sem `ConfirmModal`** — clique único na lixeira apagava o objeto do R2 permanentemente, quebrando a convenção do próprio projeto de exigir confirmação em toda exclusão destrutiva (turma, aluno, tag, tipo, biblioteca já usam `ConfirmModal`/`confirmState`); **(2) mais grave — delete rodava antes do save**: `handleRemoveUploadedVideo` chamava a Edge Function no clique da lixeira, antes do submit do formulário; se o admin clicasse "Cancelar" depois de apagar (ou fechasse o painel, caísse a conexão etc.), o objeto já tinha sumido do R2 pra sempre, mas o `video_url` no banco continuava intacto apontando pra ele — link quebrado em produção sem forma de desfazer, pior que o problema original (arquivo órfão) que a feature deveria resolver. Corrigido: exclusão real adiada para dentro de `AdminTreinos.tsx#handleSubmit`, só executada **depois** que `updateTraining`/`createTraining` confirma sucesso; o clique na lixeira agora só abre `ConfirmModal` (via `createPortal` — o painel usa `motion.div` com `transform`, mesma classe de bug do Caso 8) e, ao confirmar, apenas marca a URL em `pendingDeleteUrl` (state local, sem chamada de rede). Ver detalhes técnicos completos na seção "Upload de Vídeo — Cloudflare R2" acima. Validado: tsc 0, lint 0, 22/22 testes, build OK.

### Sessão 2026-09-21 — Reativação, diagnóstico e blindagem do keep-alive (Supabase + GitHub Actions)

Diagnóstico completo de saúde (registrado em `STATUS_PROJETO.md`) com veredito **OPERACIONAL**: Postgres ativo (queries reais retornando o `401`/`42501` esperado da role `anon`, sem `000`/`5xx`/timeout), Auth `GET /auth/v1/health` → HTTP 200, 9/9 Edge Functions no ar, e app íntegro (`lint` 0, `tsc` 0, 22/22 testes, `build` OK). Na sequência, blindagem do keep-alive anti-pausa: `.github/workflows/keep-alive.yml` agora exige **HTTP 200** tanto do Auth quanto do novo `POST /rest/v1/rpc/keepalive` (com `--max-time 20 --retry 2`), e secret `SUPABASE_ANON_KEY` ausente virou erro fatal (`::error::` + `exit 1` — antes era `warning` + `exit 0`). Nova RPC **`public.keepalive()`** (migration `supabase/migrations/20260921000000_add_keepalive_rpc.sql`): `RETURNS jsonb LANGUAGE sql SECURITY INVOKER SET search_path = ''`, `SELECT pg_catalog.jsonb_build_object('status', 'ok', 'timestamp', pg_catalog.now())`, `GRANT EXECUTE ... TO anon, authenticated, service_role` — healthcheck dedicado que força query real no Postgres sem expor tabela de negócio, eliminando a ambiguidade do ping anterior em `training_types` (401 esperado). **Ordem de deploy:** aplicar a migration (`npx supabase db push`) **antes** de disparar o workflow atualizado, senão o `POST /rpc/keepalive` retorna 404 e o workflow falha vermelho por motivo falso. Detalhes em `CLAUDE_HISTORICO.md` (sessão 2026-09-21) e lições em `GEMINI_LESSONS.md` (itens 16–17).
...
- **Sessão 2026-07-12:** 3 entregas — (1) **feat:** feature de check-in integrado ao Strava — botão "Importar do Strava" em `CheckinSheet.tsx`, novo hook `useStravaActivitiesLocal.ts`, `checkins.strava_activity_id` (ver seção "Integração Strava" acima); (2) **fix:** `GRANT INSERT, UPDATE ON strava_activities TO service_role` — sincronização do Strava vinha falhando silenciosamente há 8 dias desde a Task 68 por falta desse GRANT (Caso 13 em `docs/PORTFOLIO_DEBUG_CASES.md`); (3) **fix:** `SetPassword.tsx` trata link de convite expirado (`otp_expired`) em vez de travar em "Validando convite..." infinito, e resolve violação de lint `react-hooks/set-state-in-effect` que bloqueava o CI da Vercel (Caso 14 em `docs/PORTFOLIO_DEBUG_CASES.md`).
- **Sessão 2026-07-13 — Limpeza de repositório + loop completo de feedback Strava/professor/aluno (6 commits):** `e0dfb39` remove artefatos indevidos comitados por engano no dia anterior (4 `.docx` do professor — desversionados, mantidos localmente —, `supabase/.temp/*`, um patch de debug órfão nunca aplicado, `tsconfig.tsbuildinfo`); `3f8e2af` remove `vite.config.js`/`vite.config.d.ts` — cópias compiladas de `vite.config.ts` que estavam comitadas e **sombreavam o `.ts` na resolução de config do Vite** (`.js` tem prioridade sobre `.ts` na ordem de busca), risco de edições futuras no `.ts` serem silenciosamente ignoradas; `042171b` `.gitignore` atualizado para prevenir recorrência de todos esses artefatos; `cdf8ccd` `CLAUDE.md` atualizado (estava desatualizado em relação ao código real da sessão externa); `0adfeff` **dados ricos do Strava no detalhe do check-in** — `CheckinDetailModal.tsx` novo, lê `strava_activities.raw` sob demanda (`useStravaActivityRaw.ts`) e exibe FC média/máxima, cadência, elevação e "Esforço Relativo" (suffer score) só quando o campo existir no JSON — e **feedback real do professor** (`checkins.professor_feedback`/`professor_feedback_at`), substituindo um `<textarea>` decorativo que existia desde a Task 68 e nunca salvou nada em lugar nenhum (Caso 15 em `docs/PORTFOLIO_DEBUG_CASES.md`); `bb52b43` **feedback visível pro aluno** — `CheckinDetailModal` promovido para `components/shared/` (prop `readOnly`, cross-role como `PaceCalculator.tsx`), badge "💬 Feedback do professor" na aba Evolução enquanto `professor_feedback_seen_at` for `null`, marcado ao abrir o check-in (fire-and-forget, só atualiza estado local se o `UPDATE` confirmar). Fecha o loop: Strava sincroniza → professor vê dado rico + escreve feedback → aluno é notificado → aluno lê. Ver seções "Convenções" (campos de `checkins`) e "Integração Strava" acima. `f23d343` **aba "Calendário" deixa de ser placeholder** — investigação prévia (sem código) mapeou 3 propostas de escopo (timeline simples / cards agregados por ciclo / calendário mensal visual); as duas últimas exigiriam generalizar o cálculo de ciclo do `useWeeklyPlan.ts` para ciclos passados (hoje só calcula o ciclo atual) e, no modo flexível, reconstruir `scheduled_day_of_week` histórico por aluno — adiadas por esforço/risco desproporcional a um MVP. Implementada só a Proposta 1 (timeline): `useProgresso.ts` ganhou `allCheckins` (lista completa já buscada, sem o `.slice(0,5)` de `recentCheckins`, que continua intocado); `AlunoCalendario.tsx` novo — página lazy-loaded no mesmo padrão de `AlunoChat`/`AlunoProgresso`/`AlunoPerfil` (busca os próprios dados via `studentId`, chunk próprio carregado só ao abrir a aba) — agrupa os checkins por mês com resumo em memória (`X treinos · Y km`, sem query nova) e reaproveita o CSS do `historyCard` de `AlunoProgresso.module.css` e o `CheckinDetailModal` já existente (`readOnly`), sem duplicar estilo nem criar modal novo.
- **Sessão 2026-09-21 — Reativação, diagnóstico e blindagem do keep-alive:** veredito OPERACIONAL (Postgres ativo, Auth HTTP 200, 9/9 Edge Functions, lint/tsc/testes/build verdes — ver `STATUS_PROJETO.md`); `.github/workflows/keep-alive.yml` agora exige HTTP 200 do Auth + `POST /rest/v1/rpc/keepalive` com `--max-time 20 --retry 2`, e secret ausente é erro fatal; nova RPC **`public.keepalive()`** (migration `20260921000000_add_keepalive_rpc.sql`, `SECURITY INVOKER`, `SET search_path = ''`, grant a `anon`/`authenticated`/`service_role`) substitui o ping em tabela de negócio — aplicar a migration antes de disparar o workflow. Ver subsection "Sessão 2026-09-21" acima e `CLAUDE_HISTORICO.md`.
- **Decisão consciente — UX do "dashboard do aluno novo" pausada por ora:** durante a investigação da aba Calendário, ficou claro que a experiência de um aluno recém-cadastrado (sem histórico, sem check-ins) ainda é pouco guiada em várias telas. Max decidiu **não** iterar mais em UX às cegas e priorizar testar o app real com o professor primeiro — qualquer redesenho de onboarding/empty states deve vir depois, informado por uso real, não suposição. Não tratar isso como esquecido nas próximas sessões: é prioridade adiada de propósito.
- **Próxima sessão:**
