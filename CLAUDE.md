...
- **Sessão 2026-07-12:** 3 entregas — (1) **feat:** feature de check-in integrado ao Strava — botão "Importar do Strava" em `CheckinSheet.tsx`, novo hook `useStravaActivitiesLocal.ts`, `checkins.strava_activity_id` (ver seção "Integração Strava" acima); (2) **fix:** `GRANT INSERT, UPDATE ON strava_activities TO service_role` — sincronização do Strava vinha falhando silenciosamente há 8 dias desde a Task 68 por falta desse GRANT (Caso 13 em `docs/PORTFOLIO_DEBUG_CASES.md`); (3) **fix:** `SetPassword.tsx` trata link de convite expirado (`otp_expired`) em vez de travar em "Validando convite..." infinito, e resolve violação de lint `react-hooks/set-state-in-effect` que bloqueava o CI da Vercel (Caso 14 em `docs/PORTFOLIO_DEBUG_CASES.md`). Também: limpeza de artefatos indevidos comitados por engano (4 `.docx` do professor, `supabase/.temp/*`, um patch de debug órfão, `tsconfig.tsbuildinfo`) e remoção de `vite.config.js`/`vite.config.d.ts` — cópias compiladas de `vite.config.ts` que estavam comitadas e **sombreavam o `.ts` na resolução de config do Vite** (`.js` tem prioridade sobre `.ts`), risco de edições futuras no `.ts` serem silenciosamente ignoradas. `.gitignore` atualizado para prevenir recorrência de todos esses artefatos.
- **Próxima sessão:**
